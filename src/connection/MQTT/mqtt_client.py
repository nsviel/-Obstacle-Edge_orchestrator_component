...
def test_sncf_connection():
    connected = param_edge.state_edge["cloud_operator"]["broker_connected"]
    if(connected == False):
        try:
            create_client()
            mqtt_connection()
...
def create_client():
    client_name = param_edge.state_edge["cloud_operator"]["mqtt_client"]
...
def mqtt_connection():
    ip = param_edge.state_edge["cloud_operator"]["broker_ip"]
    port = param_edge.state_edge["cloud_operator"]["broker_port"]
    param_edge.mqtt_client.connect(ip, port)
    param_edge.state_edge["cloud_operator"]["broker_connected"] = True
