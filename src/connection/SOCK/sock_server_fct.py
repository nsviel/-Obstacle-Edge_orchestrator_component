...
def thread_socket_l1_server():
    port = param_edge.state_edge_1["self"]["sock_server_l1_port"]
    ...
    while param_edge.run_thread_socket:
        try:
            ...
            param_edge.state_edge_1["module_capture"]["sock_l1_connected"] = True
...
def process_l1_data(data):
    if(param_edge.state_edge_1["self"]["lidar_main"] == "lidar_1"):
        sock_client.send_packet_l1(data)
    elif(param_edge.state_edge_1["self"]["lidar_main"] == "lidar_2"):
